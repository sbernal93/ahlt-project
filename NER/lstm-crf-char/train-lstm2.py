...
if __name__ == '__main__':
    ...
    main_lstm = Bidirectional(LSTM(units=50, return_sequences=True,
                           recurrent_dropout=0.8))(x)
    out = TimeDistributed(Dense(100, activation="softmax"))(main_lstm)
    #crf = CRF(n_tags)  # CRF layer
    #out = crf(model)  # output

    model = Model([word_in, char_in], out)
    #model.compile(optimizer="adam", loss=crf.loss_function, metrics=[crf.accuracy])
    optimizer = RMSprop(lr=0.03, rho=0.9, epsilon=None, decay=0.0)
    model.compile(optimizer=optimizer, loss="sparse_categorical_crossentropy")

    history = model.fit([X, np.array(X_char).reshape((len(X_char), max_len, max_len_char))],
                        np.array(Y).reshape(len(Y), max_len, 1), batch_size=64, epochs=15,
    ...
    for xseq,toks in instances_pred(open(sys.argv[2])):
        ...
        x_test_sent = pad_sequences(maxlen=max_len, sequences=x_test_sent, value=tag2idx["PAD"], padding='post', truncating='post')
